...
data = [
    ...
    for line in open(os.path.join(os.path.dirname(__file__), '..', 'data', 'training_data.txt'), encoding="utf-8")
]
...
train_doc_bin.to_disk(os.path.join(os.path.dirname(__file__), '..', 'data', 'train.spacy'))


dev_doc_bin = convert_to_spacy_format(dev_data, nlp)
dev_doc_bin.to_disk(os.path.join(os.path.dirname(__file__), '..', 'data', 'dev.spacy'))

test_doc_bin = convert_to_spacy_format(test_data, nlp)
test_doc_bin.to_disk(os.path.join(os.path.dirname(__file__), '..', 'data', 'test.spacy'))